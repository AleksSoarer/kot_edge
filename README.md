...
В версии 0.10.0 под мордочкой добавлены восемь фиксированных индикаторов
`M0…M7`. Voice-worker получает от SoX все каналы массива после ресемплинга,
вычисляет их RMS с учётом текущего `KOT_MIC_GAIN`, но в VAD/ASR по-прежнему
отправляет только выбранный `KOT_MIC_BEAM_CHANNEL`:
