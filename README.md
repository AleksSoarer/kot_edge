...
Версия 0.8.2 открывает после входа в графическую сессию одно обычное окно
...
нажимает штатную кнопку Play, после чего скрипт обнаруживает созданный MPRIS
через `playerctl` и сразу ставит музыку на паузу. Затем открывается Кот и
становится видимой вкладкой. Поэтому после загрузки музыка не играет, но уже
готова принять голосовую команду «включи музыку».
...
KOT_MUSIC_WARMUP_SECONDS=10  # максимальное ожидание MPRIS
```
