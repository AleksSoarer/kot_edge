...
Версия 0.8.4 открывает после входа в графическую сессию одно обычное окно
...
он ждёт `Playing` и непустое название трека, даёт очереди 3 секунды на
стабилизацию, ставит музыку на паузу и проверяет, что статус `Paused` и метаданные
сохранились. Только после этого открывается Кот. Поэтому после загрузки музыка
не играет, но уже готова принять голосовую команду «включи музыку».
